...
> **TODO:** Replace `<YOUR_TEST_ACCOUNT_ID>` and `<YOUR_CUSTOM_OBJECT_ID>` with your actual values from HubSpot.
...
- `PRIVATE_APP_ACCESS_TOKEN`: HubSpot account → Settings → Integrations → Private Apps → your app → Show Token
- `CUSTOM_OBJECT_TYPE`: HubSpot account → Settings → Objects → Custom Objects → your object → copy the ID (starts with `2-`)
